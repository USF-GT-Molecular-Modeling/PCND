...
class coeff:
    ...
    def set(self, **coeffs):
        ...
        #for typei in type:
        self.set_single(coeffs);

    ## \internal
    # \brief Sets a single parameter
    def set_single(self, coeffs):
        #type = str(type);
    ...
    def verify(self, required_coeffs):
        ...
        if not hoomd.init.is_initialized():
            hoomd.context.msg.error("Cannot verify PCND coefficients before initialization\n");
        ...
        count = 0;
        for coeff_name in self.values.keys():
            if not coeff_name in required_coeffs:
                hoomd.context.msg.notice(2, "Notice: Possible typo? Force coeff " + str(coeff_name) + "is not used by the PCND force\n");
            else:
                count += 1;

        if count != len(required_coeffs):
            hoomd.context.msg.error("PCND Force is missing required coefficients\n");
            valid = False;
...
class linear(force._force):
    ...
    def update_coeffs(self):
        ...
        if not self.PCND_coeff.verify(coeff_list):
        ...
        self.cpp_force.setParams(self.PCND_coeff.values['Xi'],self.PCND_coeff.values['Tau']);
        #self.cpp_force.setParams(i, coeff_dict['ChainNum'], coeff_dict['Xi'], coeff_dict['Tau']);
        print("\n\n\n\n\n UPDATING COEFFS IN PYTHON \n\n\n\n");
        # I could put the funciton to work out hte chains here
