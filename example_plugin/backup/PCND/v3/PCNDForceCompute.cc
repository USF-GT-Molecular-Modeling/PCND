...
PCNDForceCompute::PCNDForceCompute(std::shared_ptr<SystemDefinition> sysdef)
    :  ForceCompute(sysdef), m_ChainNum(NULL), m_Xi(NULL), m_Tau(NULL)
    {
...
*/
void PCNDForceCompute::setParams(unsigned int type, unsigned int ChainNum, Scalar Xi, Scalar Tau)
    {
    // make sure the type is valid
    if (type >= m_angle_data->getNTypes())
        {
        m_exec_conf->msg->error() << "PCND: Invalid angle type specified" << endl;
        throw runtime_error("Error setting parameters in PCNDForceCompute");
        }

    m_ChainNum[type] = ChainNum;
    m_Xi[type] = Xi;
    m_Tau[type] = Tau;

    // check for some silly errors a user could make
    if (ChainNum < 0)
        m_exec_conf->msg->warning() << "PCND: specified ChainNum <= 0" << endl;
    if (Xi <= 0)
