...
/*! \file HarmonicAngleForceComputePCND.cc
    \brief Contains code for the HarmonicAngleForceComputePCND class
*/

/*! \param sysdef System to compute forces on
    \post Memory is allocated, and forces are zeroed.
*/
HarmonicAngleForceComputePCND::HarmonicAngleForceComputePCND(std::shared_ptr<SystemDefinition> sysdef)
    ...
    if (m_angle_data->getNTypes() == 0)
        ...
        throw runtime_error("Error initializing HarmonicAngleForceComputePCND");
        }
...
HarmonicAngleForceComputePCND::~HarmonicAngleForceComputePCND()
    {
    m_exec_conf->msg->notice(5) << "Destroying HarmonicAngleForceComputePCND" << endl;
...
*/
void HarmonicAngleForceComputePCND::setParams(unsigned int type, Scalar K, Scalar t_0)
    {
    // make sure the type is valid
    if (type >= m_angle_data->getNTypes())
        {
        m_exec_conf->msg->error() << "angle.harmonic: Invalid angle type specified" << endl;
        throw runtime_error("Error setting parameters in HarmonicAngleForceComputePCND");
        }
...
*/
std::vector< std::string > HarmonicAngleForceComputePCND::getProvidedLogQuantities()
    {
    vector<string> list;
...
*/
Scalar HarmonicAngleForceComputePCND::getLogValue(const std::string& quantity, unsigned int timestep)
    {
 ...
 */
void HarmonicAngleForceComputePCND::computeForces(unsigned int timestep)
    {
...
void export_HarmonicAngleForceComputePCND(py::module& m)
    {
    py::class_<HarmonicAngleForceComputePCND, std::shared_ptr<HarmonicAngleForceComputePCND> >(m, "HarmonicAngleForceComputePCND", py::base<ForceCompute>())
    .def(py::init< std::shared_ptr<SystemDefinition> >())
    .def("setParams", &HarmonicAngleForceComputePCND::setParams)
    ;
    }
