...
/*! \file PCNDHarmonicAngleForceGPU.cu
    \brief Defines GPU kernel code for calculating the harmonic angle forces. Used by PCNDHarmonicAngleForceComputeGPU.
*/
...
extern "C" __global__ void gpu_compute_harmonic_angle_forces_kernel(Scalar4* d_force,
    ...
    /*
		if (idx ==0)
		{
			printf("Timestep = %i Device=",timestep);
			for (int i = 0; i<6; i++)
			{
    ...
    */
		...
		for (int angle_idx = 0; angle_idx < n_angles; angle_idx++)
			...
			//printf("\nidx = %i\ndevData[0] = %f",idx,hostData);
			float R1 = hostData[idx*6];//curand_uniform(&localState);
			float R2 = hostData[idx*6+1];
			float R3 = hostData[idx*6+2];
			float R4 = hostData[idx*6+3];
			float R5 = hostData[idx*6+4];
			float R6 = hostData[idx*6+5];
			//state[idx] = localState;
			//if (idx ==0 || idx ==1)
			//printf("\ntimestep = %i idx = %i R1 = %f  R6 = %f\n",timestep,idx,R1,R6);
...
cudaError_t gpu_compute_harmonic_angle_forces(Scalar4* d_force,
    ...
    /* Show result */
    //printf("Timestep = %i Host=",timestep);
    //for(i = 0; i < 6; i++) {
        //printf("%1.4f ", hostData[i]);
    //}
    //printf("\n\n\n");
    ...
    gpu_compute_harmonic_angle_forces_kernel<<< grid, threads>>>(d_force, d_virial, virial_pitch, N, d_pos, d_params, box,
        atable, apos_list, pitch, n_angles_list,timestep,devData);
