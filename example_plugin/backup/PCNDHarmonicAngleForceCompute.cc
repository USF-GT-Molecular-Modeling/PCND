...
/*! \file PCNDHarmonicAngleForceCompute.cc
    \brief Contains code for the PCNDHarmonicAngleForceCompute class
*/

/*! \param sysdef System to compute forces on
    \post Memory is allocated, and forces are zeroed.
*/
PCNDHarmonicAngleForceCompute::PCNDHarmonicAngleForceCompute(std::shared_ptr<SystemDefinition> sysdef)
    :  ForceCompute(sysdef), m_K(NULL), m_t_0(NULL)
    {
    m_exec_conf->msg->notice(5) << "Constructing PCNDHarmonicAngleForceCompute" << endl;
    ...
    if (m_angle_data->getNTypes() == 0)
        ...
        throw runtime_error("Error initializing PCNDHarmonicAngleForceCompute");
        }
...
PCNDHarmonicAngleForceCompute::~PCNDHarmonicAngleForceCompute()
    {
    m_exec_conf->msg->notice(5) << "Destroying PCNDHarmonicAngleForceCompute" << endl;
...
*/
void PCNDHarmonicAngleForceCompute::setParams(unsigned int type, Scalar K, Scalar t_0)
    {
    // make sure the type is valid
    if (type >= m_angle_data->getNTypes())
        {
        m_exec_conf->msg->error() << "angle.harmonic: Invalid angle type specified" << endl;
        throw runtime_error("Error setting parameters in PCNDHarmonicAngleForceCompute");
        }
...
*/
std::vector< std::string > PCNDHarmonicAngleForceCompute::getProvidedLogQuantities()
    {
    vector<string> list;
...
*/
Scalar PCNDHarmonicAngleForceCompute::getLogValue(const std::string& quantity, unsigned int timestep)
    {
 ...
 */
void PCNDHarmonicAngleForceCompute::computeForces(unsigned int timestep)
    {
...
void export_PCNDHarmonicAngleForceCompute(py::module& m)
    {
    py::class_<PCNDHarmonicAngleForceCompute, std::shared_ptr<PCNDHarmonicAngleForceCompute> >(m, "PCNDHarmonicAngleForceCompute", py::base<ForceCompute>())
    .def(py::init< std::shared_ptr<SystemDefinition> >())
    .def("setParams", &PCNDHarmonicAngleForceCompute::setParams)
    ;
    }
